...
public:
	void mutar(Cromosoma* c){
		...
		if (myRandom::getRandom(0u, 1u)){
			...
			bool ok = false;
			do{
				randNodo = myRandom::getRandom(0u, ady.size() - 1);
				randNodo2 = myRandom::getRandom(0u, ady.size() - 1);
			...
			do{
				...
				if (vecinos > 0){
					randArista = myRandom::getRandom(0u, vecinos - 1);
		...
		if (myRandom::getRandom(0u, 1u)){
			...
			for (size_t i = 0; i < roles.size(); ++i){
				if (roles[i] = randNodo){
					...
					do{
						ok = true;
						roles[i] = myRandom::getRandom(0, int(ady.size() - 1));
...
public:
	void mutar(Cromosoma* c){
		...
		if (g.size() <= 3) return;	// no se puede hacer esta mutacion con 3 menos nodos
		auto roles = c->getRolesSala();
		unsigned int randRol = myRandom::getRandom(0u, roles.size() - 1);
		...
		do{
			ok = true;
			do{
				randNodo = myRandom::getRandom(0u, g.size() - 1);
...
public:
	void mutar(Cromosoma* c){
		Grafo<Gen> g = c->getGenotipo();
		unsigned int opcion = myRandom::getRandom(0u, 2u);
		if (opcion == 0){
			// Mutacion Nodo
			if (myRandom::getRandom(0u, 1u)){
				// Se crea una nuevo nodo
