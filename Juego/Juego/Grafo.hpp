...
public:
	...
	{
		std::srand(time(NULL));
		unsigned int nNodos = myRandom::getRandom(minNodos, maxNodos);
		...
		while (nAristas > 0){
			nodoA = myRandom::getRandom(0u, nNodos - 1);
			nodoB = myRandom::getRandom(0u, nNodos - 1);
	...
	static Grafo<N> unirGrafo(Grafo<N> a, Grafo<N> b, double densidad = 0.1){
		...
		if (nodosA.size() > 0 && nodosB.size() > 0){
			...
			if (sizeA == 1 && nodosB.size() == 1){
				...
				if (n == 0) n = 1;
				nuevas = myRandom::getRandom(1u, n);	// Para evitar demasiada densidad, se limita al 75% de las posibles nuevas aristas
			}
			std::vector<std::pair<unsigned int, unsigned int>> uniones;
			for (std::size_t i = 0; i < sizeA; ++i){
				for (std::size_t j = 0; j < nodosB.size(); ++j){
					uniones.push_back(std::pair<unsigned int, unsigned int>(i, j + sizeA));
				}
			}
	...
	*/
	...
	std::vector<Grafo<N>> divideEnGrafos(unsigned int n){
		...
		aux = divideGrafo(myRandom::getRandom(0, (int)_nodos.size() - 1));
		for (std::size_t i = 1; i < n; ++i){
			if (i == n - 1){
				...
				if (aux[0].size() < aux[1].size()){
					ret.push_back(aux[0]);
					aux = aux.at(1).divideGrafo(myRandom::getRandom(0, (int)aux[1].size() - 1));
				}
				else{
					ret.push_back(aux[1]);
					aux = aux.at(0).divideGrafo(myRandom::getRandom(0, (int)aux[0].size() - 1));
				}
			}
		}
		return ret;
