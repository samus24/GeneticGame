...
public:
	void seleccionar(Poblacion* pob) {
		...
		prob = myRandom::getRandom(0.f, 1.f) * seg;
		for (int i = 0; i < pob->_tam; ++i){
			posSuper = 0;
			while ((posSuper < pob->_tam) && (prob > pob->individuos[posSuper].getPuntAcum()))
				posSuper++;
...
public:
	void seleccionar(Poblacion* pob) {
		...
		for (int i = 0; i < pob->_tam; ++i){
			...
			indexA = myRandom::getRandom(0u, pob->_tam - 1);
			do{
				indexB = myRandom::getRandom(0u, pob->_tam - 1);
			} while (indexB == indexA);
			do{
				indexC = myRandom::getRandom(0u, pob->_tam - 1);
